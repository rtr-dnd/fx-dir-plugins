	...
	function follow_button( $user_id1, $user_id2 ) {
		...
		if ( $this->can_follow( $user_id1, $user_id2 ) ) {
			...
			if ( ! $this->followed( $user_id1, $user_id2 ) ) {
				$res = '<a href="javascript:void(0);" class="um-follow-btn um-button um-alt" data-user_id1="' . esc_attr( $user_id1 ) . '" data-user_id2="' . esc_attr( $user_id2 ) . '">'. __( 'Follow', 'um-followers' ). '</a>';
			} else {
				$res = '<a href="javascript:void(0);" class="um-unfollow-btn um-button" data-user_id1="' . esc_attr( $user_id1 ) . '" data-user_id2="' . esc_attr( $user_id2 ) . '" data-following="' . esc_attr__( 'Following', 'um-followers' ).'"  data-unfollow="' . esc_attr__( 'Unfollow', 'um-followers' ) . '">' . __( 'Following', 'um-followers' ) . '</a>';
			}
