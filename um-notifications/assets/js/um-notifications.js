...
function um_load_notifications() {
	...
	jQuery.ajax({
		...
		success: function( response ) {
			...
			var um_notification_icon = jQuery('.um-notification-b');
			...
			if ( response.data.refresh_count ) {
				...
				jQuery( document ).trigger( 'um_notification_refresh_count' );
				um_animate_bubble();
			}
			...
			if ( response.data.unread ) {
				...
				if ( jQuery('.um-notification[data-notification_id="' + notification_id + '"]').length === 0 ) {
					...
					jQuery('.um-notification[data-notification_id="' + notification_id + '"]').removeClass('read').addClass('unread');
				}
