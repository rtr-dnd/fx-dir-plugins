...
function um_request_verification() {
	...
	if ( isset( $_REQUEST['request_verification'] ) && isset( $_REQUEST['uid'] ) ) {
		...
		exit( wp_redirect( esc_url_raw( $_REQUEST['redirect_to'] ) ) );
	}
...
function um_update_verification_field( $user_id ) {
	if ( current_user_can( 'edit_user', $user_id ) && isset( $_POST['um_set_verification'] ) ) {
		...
		if ( absint( $_POST['um_set_verification'] ) == 1 && $state == 0 ) {
			UM()->Verified_Users_API()->api()->verify( $user_id );
		} else if ( $state == 1 && absint( $_POST['um_set_verification'] ) == 0 ) {
			UM()->Verified_Users_API()->api()->unverify( $user_id );
		}
	}
}
 ...
 * @hook 'um_update_profile_full_name' from um_user_edit_profile()
 * @param int $user_id
 */
function um_verify_user_if_complete( $user_id ) {
	...
	if ( ! empty( $role_data['profilec_verify'] ) ) {
		...
		if ( $completeness && $completeness['progress'] >= 100 ) {
			UM()->Verified_Users_API()->api()->verify( $user_id, true );
...
add_action( 'um_update_profile_full_name',  'um_verify_user_if_complete', 20, 2 );