...
class Groups_Requests extends \um\core\Member_Directory {
	...
	function __construct() {
		...
		add_filter( 'um_get_group_user_data_requests', array( $this, 'get_group_user_data' ), 10, 2 );
	}
	...
	function ajax_get_members() {
		...
		foreach ( $user_ids as $user_id ) {
			...
			$user_data = apply_filters( 'um_get_group_user_data_requests', $user_id, $group_id );
			...
			do_action( "um_groups_users_list_after_details__requests", $user_id, $group_id, $user_data[0], $menus, $user_data[0]['has_joined'] );
