...
class Groups_Main_API {
	...
	function change_member_group_role() {
		...
		if ( ! empty( $_POST['admin'] ) ) {
			...
			wp_send_json( array( 'found' => true, 'role' => $this->group_roles[ $role ], 'role_slug' => $role, 'success_message' => __("Role changed to {$this->group_roles[ $role ]}", "um-groups"), "menus" => $roles_swap_menus, "previous_role" => $current_group_role ) );
		} else {
			UM()->check_ajax_nonce();
			...
			wp_send_json_success( array( 'role' => $this->group_roles[ $role ], 'role_slug' => $role, 'success_message' => sprintf( __( 'Role changed to %s', 'um-groups' ), $this->group_roles[ $role ] ), 'menus' => $roles_swap_menus, 'previous_role' => $current_group_role ) );
		}
	}
	...
	function search_member_suggest() {
		...
		$search = '%' . $wpdb->esc_like( sanitize_text_field( $_REQUEST['q'] ) ) .'%';
		$group_id = sanitize_key( $_REQUEST['group_id'] );

		$users = $wpdb->get_col( $wpdb->prepare(
			"SELECT ID
			FROM {$wpdb->users}
			WHERE user_login LIKE %s OR
			      user_email LIKE %s
			LIMIT 10",
			$search,
			$search
