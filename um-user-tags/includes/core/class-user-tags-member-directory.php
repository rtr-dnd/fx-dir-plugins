...
class User_Tags_Member_Directory {
	...
	function extends_search_query_meta( $query_string, $search ) {
		...
		if ( ! empty( $term->term_id ) ) {
			global $wpdb;
			$query_string .= $wpdb->prepare( ' OR umm_search.um_value LIKE %s OR umm_search.um_value LIKE %s', '%' . serialize( strval( $term->term_id ) ) . '%', '%' . serialize( strval( $term->slug ) ) . '%' );
		}
	...
	function user_tags_filter_options( $options, $values_array, $attrs ) {
		...
		$options = array_intersect_key( $attrs['options'], array_flip( $values_array ) );
